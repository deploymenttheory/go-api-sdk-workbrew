...
// LicensesMock handles mock HTTP responses for licenses
type LicensesMock struct{}

// RegisterMocks registers all success mock responses
func (m *LicensesMock) RegisterMocks(baseURL string) {
	// Mock GET /licenses.json
	httpmock.RegisterResponder("GET", baseURL+"/licenses.json",
		func(req *http.Request) (*http.Response, error) {
			mockData, err := loadMockResponse("validate_get_licenses.json")
			if err != nil {
	...
	// Mock GET /licenses.csv
	httpmock.RegisterResponder("GET", baseURL+"/licenses.csv",
		func(req *http.Request) (*http.Response, error) {
			mockData, err := loadMockResponse("validate_get_licenses.csv")
			if err != nil {
				return httpmock.NewStringResponse(500, `{"message":"Internal Server Error","errors":["Failed to load mock data"]}`), nil
			}
			resp := httpmock.NewBytesResponse(200, mockData)
...
func (m *LicensesMock) RegisterErrorMocks(baseURL string) {
...
func (m *LicensesMock) CleanupMockState() {
